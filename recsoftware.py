...
# Combined two lines  into one using List comprehension.
RestaurantList = [Restaurant(restaurant[0], restaurant[1], restaurant[2],
                             restaurant[3], restaurant[4]) for restaurant in restaurantData.restaurant_data]
...
while run:
    ...
    while letter_search:
        ...
        letter_input = input(
            "Search the beginning of the type of food you want to eat to see if it is available.\n")
    ...
    if sort_choice.lower() == 'p' or sort_choice.lower() == 'price':
        ...
        type_choice_list = search_by_type(type_input, RestaurantList)
        sort_list = sorted(
            type_choice_list, key=lambda r: r.rating, reverse=True)
        for restaurant in sort_list:
            print(lineBreak)
            print(restaurant)
        print(lineBreak)
